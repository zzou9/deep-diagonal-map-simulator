...
class TriangleMap {
    ...
    applyMap(coords, p, checkAfine) {
        ...
        let imgCoords = this.applyFactor(this.applyFactor(coords.slice(), this.l), this.k);
        ...
        //         const v = Reconstruct.reconstructTriangle7(coords);
        ...
        //         vertices = Reconstruct.reconstructTriangle(T, k+l+6);
        //     } else {
     ...
     * The map D_k on P = (x0, x1, x2, x3, x4, x5) is given as 
     * D_k(P)_i = <x(-k-i), x(-i)>
     * @param {*} coords 
     * @param {*} k 
     */
    applyFactor(coords, k) {
        ...
        const M2Inv = MathHelper.invert3(M2);
        const T = MathHelper.matrixMult(M2Inv, M1);
        let vertices = Reconstruct.reconstructTriangle(T, k+7);
        // let vertices = Reconstruct.reconstruct3(coords, k+6);
        // populate the new vertices 
        let imgVertices = new Array(7);
        for (let i = 0; i < 7; i++) {
            imgVertices[6-i] = MathHelper.cross(vertices[i], vertices[i+k]); // see formula
        }
        let tempCoords = Geometry.getCornerCoords(imgVertices);
        // populate the image corner invariants
        let imgCoords = new Array(6);
        for (let i = 0; i < 6; i++) {
            imgCoords[(4*k-4+i)%6] = tempCoords[i+4];
        }
        return imgCoords;
    }
